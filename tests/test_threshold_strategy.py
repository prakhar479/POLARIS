...
class TestThresholdReactiveStrategy:
    ...
    async def test_unknown_metric(self, strategy, context):
        ...
        state = SystemState(
            ...
            timestamp=datetime.now(timezone.utc),
            metrics={
                "unknown_metric": MetricValue("unknown_metric", 100.0, "percent")
            },
            health_status=HealthStatus.HEALTHY
        )
    ...
    async def test_invalid_metric_value(self, strategy, context):
        ...
        state = SystemState(
            ...
            timestamp=datetime.now(timezone.utc),
            metrics={
                "cpu_usage": MetricValue("cpu_usage", "invalid", "percent")
            },
            health_status=HealthStatus.HEALTHY
        )
    ...
    async def test_server_count_logic(self, strategy, context):
        ...
        state_low = SystemState(
            ...
            timestamp=datetime.now(timezone.utc),
            metrics={
                "server_count": MetricValue("server_count", 1, "count")  # Below low threshold
            },
            health_status=HealthStatus.HEALTHY
        )
        ...
        state_high = SystemState(
            ...
            timestamp=datetime.now(timezone.utc),
            metrics={
                "server_count": MetricValue("server_count", 15, "count")  # Above high threshold
            },
            health_status=HealthStatus.HEALTHY
        )
