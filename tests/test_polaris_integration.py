...
class TestPolarisIntegration:
    ...
    def test_polaris_initialization(self, polaris_config, mock_logger, mock_metrics):
        ...
        polaris = Polaris(config=polaris_config, logger=mock_logger, metrics=mock_metrics)
    ...
    async def test_monitoring_loop_execution(self, polaris, mock_connector, mock_strategy):
        ...
        sample_state = SystemState(
            ...
            metrics={"cpu_usage": MetricValue("cpu_usage", 85.0, "percent")},  # High CPU
    ...
    async def test_monitoring_loop_no_action_needed(self, polaris, mock_connector, mock_strategy):
        ...
        sample_state = SystemState(
            system_id="test-system",
            timestamp=datetime.now(timezone.utc),
            metrics={"cpu_usage": MetricValue("cpu_usage", 50.0, "percent")},  # Normal CPU
            health_status=HealthStatus.HEALTHY,
        )
    ...
    @pytest.mark.asyncio
    async def test_monitoring_loop_action_validation_failure(
        self, polaris, mock_connector, mock_strategy
    ):
    ...
    async def test_multiple_connectors(
        self, polaris_config, mock_logger, mock_metrics, mock_strategy
    ):
