...
class SwitchSystemConnector(ManagedSystemConnector):
    ...
    def __init__(self, system_config: Dict[str, Any], logger):
        ...
        # Get base path for file resolution (plugin directory)
        plugin_dir = Path(__file__).parent
        
        # Resolve file paths (use absolute paths or relative to plugin dir)
        self.model_file_path = str(plugin_dir / self.switch_config.get("model_file_path", "../switch/NAVIE/model.csv"))
        self.monitor_file_path = str(plugin_dir / self.switch_config.get("monitor_file_path", "../switch/NAVIE/monitor.csv"))
        self.knowledge_file_path = str(plugin_dir / self.switch_config.get("knowledge_file_path", "../switch/NAVIE/knowledge.csv"))
        self.metrics_file_path = str(plugin_dir / self.switch_config.get("metrics_file_path", "../switch/NAVIE/metrics.csv"))
    ...
    async def execute_command(
        ...
        """Execute a command on the Switch system.
        
        Args:
            command_template: Template name of the command to execute
        ...
        self.logger.debug(f"execute_command called: command='{command_template}', params={params}")
        ...
        # Convenience commands for individual metrics (used in preconditions)
        elif command_template == "cpu_usage":
            metrics = await self._get_latest_metrics()
            return float(metrics.get("cpu", 0.0))
        elif command_template == "image_processing_time":
            metrics = await self._get_latest_metrics()
            return float(metrics.get("image_processing_time", 0.0))
        elif command_template == "utility":
            metrics = await self._get_latest_metrics()
            return float(metrics.get("utility", 0.0))
        elif command_template == "confidence":
            metrics = await self._get_latest_metrics()
            return float(metrics.get("confidence", 0.0))
        elif command_template == "detection_boxes":
            metrics = await self._get_latest_metrics()
            return int(metrics.get("detection_boxes", 0))
        
        elif command_template == "get_latest_metrics":
            # Get all latest metrics
            metrics_dict = await self._get_latest_metrics()
            
            # DEBUG: Log what we received
            self.logger.debug(f"get_latest_metrics called with params: {params}")
            self.logger.debug(f"Retrieved metrics_dict: {list(metrics_dict.keys()) if metrics_dict else 'None'}")
            
            # If a specific metric name is requested via params, extract just that value
            if params and "metric_name" in params:
                metric_name = params["metric_name"]
                self.logger.debug(f"Extracting metric '{metric_name}' from response")
                if metric_name in metrics_dict:
                    value = metrics_dict[metric_name]
                    self.logger.debug(f"Extracted value: {value} (type: {type(value).__name__})")
                    return value
                else:
                    self.logger.warning(f"Metric '{metric_name}' not found in response. Available: {list(metrics_dict.keys())}")
                    return None
            
            # Otherwise return the full dictionary
            self.logger.warning(f"No metric_name in params, returning full dictionary (this may cause errors)")
            return metrics_dict
    ...
    async def _get_metrics(self, params: Optional[Dict[str, Any]]) -> Dict[str, Any]:
        """Get system metrics.
    ...
    async def get_performance_metrics(self) -> Dict[str, float]:
        ...
        try:
            ...
            self.logger.debug("Retrieved performance metrics", extra={"metrics": metrics})
