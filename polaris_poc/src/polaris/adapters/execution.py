...
class ExecutionAdapter(ExternalAdapter):
    ...
    def _validate_action_parameters(self, action: ControlAction, action_config: Dict[str, Any]) -> Optional[str]:
    ...
    async def _evaluate_expression(self, expression: str) -> Union[str, int, float, bool]:
        ...
        self.logger.debug(
            "Evaluating expression as a command",
    ...
    async def execute_action(self, action: ControlAction) -> ExecutionResult:
        ...
        try:
            ...
            response = await self.connector.execute_command(
                command_template,
                action.params
            )
    ...
    async def _enqueue_action(self, action: ControlAction):
        ...
        try:
            ...
            await self._publish_metric("queue_length", {
                "queue_length": queue_size_after
            })
                ...
                async with self._execution_semaphore:
                    ...
                    try:
                        ...
                        self.logger.exception(
                            "Unexpected error in action processing",
    ...
    async def _stop_processing(self) -> None:
        ...
        try:
            ...
            self.logger.warning(
                "Action queue drain timeout",
                extra={"remaining": self.action_queue.qsize()}
            )
