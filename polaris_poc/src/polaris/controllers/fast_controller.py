...
class FastController(BaseController):
    ...
    def decide_action(self, telemetry_snapshot: dict):
        ...
        if weighted_response_time > self.RT_THRESHOLD:
            ...
            self.logger.info(
                f"High RT detected. Violation count: {self.violation_counter}/{self.PERSISTENCE_COUNT}"
            )

            # --- MODIFIED: Act only after persistence threshold is met ---
            if self.violation_counter >= self.PERSISTENCE_COUNT:
                self.logger.warning(
                    f"RT threshold violated for {self.violation_counter} consecutive checks. Taking action."
                )
    ...
    def _handle_normal_response_time(self, telemetry_values):
        ...
        avg_utilization = (
            telemetry_values.get("swim.server.utilization", 0) / active_servers
            if active_servers > 0
            else 0
        )
        ...
        # --- MODIFIED: Less aggressive scale-down logic ---
            ...
            current_dimmer == 1.0
            and active_servers > 1
            and avg_utilization < self.LOW_UTIL_THRESHOLD
        ):
            self.logger.info(
                f"Normal RT, low utilization ({avg_utilization:.2f} < {self.LOW_UTIL_THRESHOLD}). Action: REMOVE_SERVER"
            )
            return self._create_action(
                "REMOVE_SERVER", {"server_type": "compute", "count": 1}, "low"
            )
