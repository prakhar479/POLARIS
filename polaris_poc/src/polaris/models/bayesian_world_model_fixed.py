...
class BayesianWorldModel(WorldModel):
    ...
    def _safe_float_conversion(self, value: Any, default: float = 0.0, param_name: str = "unknown") -> float:
        ...
        try:
            ...
            self.logger.warning(f"Failed to convert parameter '{param_name}' value '{value}' to float: {e}, using default {default}")
            return default
    ...
    async def _apply_enhanced_action_effects(self, future_state: Dict, actions: List[Any], time_step: int, current_model: str) -> Tuple[Dict, Dict]:
        ...
        for i, action in enumerate(actions):
            ...
            if isinstance(action, dict):
                ...
                self.logger.debug(f"_apply_enhanced_action_effects: Dict action - type: {action_type}, params: {action_params}")
            elif hasattr(action, 'action_type'):
                action_mode = action.action_type
                action_params = getattr(action, 'params', {})
                self.logger.debug(f"_apply_enhanced_action_effects: Object action - type: {action_type}, params: {action_params}")
            else:
            ...
            if action_type and ("SWITCH_MODEL" in action_type or action_type == "SWITCH_MODEL"):
                ...
                self.logger.debug(f"_apply_enhanced_action_effects: SWITCH_MODEL action - target: {target_model}, current: {current_model}")
    ...
    async def _estimate_enhanced_action_impacts(self, actions: List[Any], future_states: List[Dict]) -> Dict[str, float]:
        ...
        initial_utility = future_states[0]["state"].get("utility", {}).get("predicted_value", 0.5)
        final_utility = future_states[-1]["state"].get("utility", {}).get("predicted_value", 0.5)
    ...
    def _aggregate_monte_carlo_scenarios(self, scenarios: List[List[Dict]], time_steps_array: np.ndarray) -> List[Dict]:
        ...
        for t in range(n_timesteps):
            ...
            for state in timestep_states:
                all_metrics.update(state["state"].keys())
            
            for metric in all_metrics:
                metric_values = []
                for state in timestep_states:
                    if metric in state["state"]:
                        metric_values.append(state["state"][metric]["predicted_value"])
                
                if metric_values:
                    aggregated_state["state"][metric] = {
                        "predicted_value": np.mean(metric_values),
                        "uncertainty": np.std(metric_values),
    ...
    def _calculate_monte_carlo_uncertainty(self, scenarios: List[List[Dict]]) -> float:
        ...
        for scenario in scenarios:
            if scenario and "utility" in scenario[-1]["state"]:
                scenario_utilities.append(scenario[-1]["state"]["utility"]["predicted_value"])
