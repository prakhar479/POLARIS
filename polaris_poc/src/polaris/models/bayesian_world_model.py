        ...
        try:
            ...
            for step in range(1, time_steps + 1):
                ...
                for metric_name, metric_state in self.system_state.metrics.items():
                    ...
                    base_confidence = max(0.1, 1.0 - (prediction_uncertainty / (abs(predicted_value) + 1.0)))
                ...
                if request.actions:
                    future_state = await self._apply_action_effects(future_state, request.actions, step)
            ...
            self.logger.debug(f"Simulation completed: {request.simulation_id} (confidence: {overall_confidence:.2f})")
            return response
            ...
            if hasattr(action, 'action_type'):
                ...
                if action_type == "ADD_SERVER":
                    ...
                    if "response_time" in future_state:
                        ...
                        future_state["response_time"]["predicted_value"] = current_rt * (0.5 + 0.5 * dimmer_value)
    ...
    async def diagnose(self, request: DiagnosisRequest) -> DiagnosisResponse:
        ...
        try:
            ...
            if anomaly_metrics:
                ...
                hypotheses.append(f"Primary anomaly detected in {primary_anomaly['metric']} with z-score {primary_anomaly['z_score']:.2f}")
                supporting_evidence.append(f"Statistical deviation: {primary_anomaly['z_score']:.2f} standard deviations from expected value")
                
                # Check for correlated anomalies
                for other_anomaly in anomaly_metrics[1:]:
                    correlation_key = tuple(sorted([primary_anomaly['metric'], other_anomaly['metric']]))
                    if correlation_key in self.system_state.correlations:
                        correlation = self.system_state.correlations[correlation_key]
                        if abs(correlation) > self.correlation_threshold:
                            hypotheses.append(f"Correlated anomaly in {other_anomaly['metric']} (correlation: {correlation:.2f})")
                            supporting_evidence.append(f"Strong correlation ({correlation:.2f}) suggests cascading effect")
            ...
            if self.system_state.system_health_score < 0.7:
                ...
                supporting_evidence.append(f"System health score: {self.system_state.system_health_score:.2f}")
            
            # Generate causal chain
            causal_chain = " -> ".join(hypotheses[:3]) if hypotheses else "No clear causal chain identified"
