...
class DigitalTwinConfigManager:
    ...
    def load_configuration(
        ...
        try:
            ...
            self.framework_config = config_manager.load_framework_config(
                framework_config_path, validate_config=validate_config
            )
            ...
            self.logger.info(
                ...
                extra={
                    "world_model_implementation": dt_config.get("world_model", {}).get(
                        "implementation"
                    ),
            ...
            raise DigitalTwinConfigError(
                f"Failed to load Digital Twin configuration: {str(e)}"
            ) from e

    def _load_world_model_config(
        self, config_path: Union[str, Path], validate_config: bool = True
    ) -> None:
        ...
        if validate_config:
            try:
                validation_result = self.validator.validate_config_file(
                    config_path=config_path, config_type="world_model"
                )
                ...
                if validation_result.valid:
                    ...
                    raise DigitalTwinConfigError(
                        f"World Model configuration validation failed: {error_msg}"
                    )
        ...
        try:
            ...
            self.logger.info(
                ...
                extra={"validation_enabled": validate_config},
            )
    ...
    def _apply_world_model_env_overrides(self) -> None:
        ...
        if implementation == "gemini":
            ...
            if (
                "gemini_model_override" in overrides
                and overrides["gemini_model_override"] in os.environ
            ):
            ...
            if (
                "api_key_env_override" in overrides
                and overrides["api_key_env_override"] in os.environ
            ):
            ...
            if (
                "vector_store_provider_override" in overrides
                and overrides["vector_store_provider_override"] in os.environ
            ):
    ...
    def _validate_configuration(self) -> None:
        ...
        if "implementation" not in wm_config:
            raise DigitalTwinConfigError(
                "Missing World Model implementation in digital_twin.world_model"
            )
    ...
    def _validate_gemini_config(self) -> None:
        ...
        if "vector_store" in config_section:
            ...
            if "provider" not in vector_config:
                raise DigitalTwinConfigError(
                    "Vector store configuration is missing 'provider' field"
                )
    ...
    def create_model_config_summary(self) -> Dict[str, Any]:
        ...
        summary = {
            ...
            "grpc_port": dt_config.get("grpc", {}).get("port"),
            "performance": {
                "max_concurrent_queries": dt_config.get("performance", {}).get(
                    "max_concurrent_queries"
                ),
                "query_timeout_sec": dt_config.get("performance", {}).get("query_timeout_sec"),
                "simulation_timeout_sec": dt_config.get("performance", {}).get(
            ...
            if world_model_config_path:
                ...
                if "world_model" in dt_config and "config_path" in dt_config["world_model"]:
                    ...
                    if Path(wm_path).exists():
                        file_configs.append(
                            {"path": wm_path, "config_type": "world_model", "schema_path": None}
                        )
    ...
    def validate_active_configuration(self) -> Optional[ValidationResult]:
        ...
        try:
            ...
            framework_result = self.validator.validate_config_dict(
                config={"digital_twin": dt_config}, config_type="framework"
            )
            ...
            if self.world_model_config:
                world_model_result = self.validator.validate_config_dict(
                    config=self.world_model_config, config_type="world_model"
                )
