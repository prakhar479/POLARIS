...
class DigitalTwinAgent:
    ...
    async def start(self) -> None:
        ...
        try:
            ...
            self.logger.error("Failed to start Digital Twin Agent", extra={"error": str(e)})
            await self.stop()
            raise
    ...
    async def stop(self) -> None:
        ...
        if self.world_model:
            try:
                ...
                self.logger.error("Error shutting down World Model", extra={"error": str(e)})
    ...
    async def _load_world_model(self) -> None:
        ...
        try:
            ...
            self.world_model = WorldModelFactory.create_model(
                model_type=model_type, config=model_config, logger=self.logger
            )
            ...
            self.logger.error(
                ...
                extra={"model_type": model_type, "error": str(e), "error_type": type(e).__name__},
            )
    ...
    async def _send_to_dead_letter_queue(self, msg: Msg, message_type: str, error: str) -> None:
        ...
        try:
            ...
            await self.nats_client.publish_json(subject=error_subject, data=error_envelope)
            ...
            self.logger.error(
                ...
                extra={"original_error": error, "dlq_error": str(dlq_error)},
            )
    ...
    async def _start_nats_ingestion(self) -> None:
        ...
        try:
            ...
            telemetry_batch_subject = telemetry_config.get(
                "batch_subject", "polaris.telemetry.events.batch"
            )
            telemetry_stream_subject = telemetry_config.get(
            ...
            execution_result_subject = execution_config.get(
            ...
            calibrate_subject = self.nats_config.get(
            ...
            self.logger.error("Failed to start NATS ingestion engine", extra={"error": str(e)})
            raise
    ...
    async def _stop_nats_ingestion(self) -> None:
        ...
        try:
            ...
            self.logger.error("Error stopping NATS ingestion engine", extra={"error": str(e)})
    ...
    async def _handle_telemetry_batch_message(self, msg: Msg) -> None:
        ...
        try:
            ...
            self.logger.error(
                ...
                extra={"error": str(e), "subject": msg.subject, "reply": msg.reply},
            )
    ...
    async def _handle_telemetry_stream_message(self, msg: Msg) -> None:
        ...
        try:
            ...
            knowledge_event = KnowledgeEvent(
                ...
                event_type="telemetry",
                data=telemetry_event,
                metadata={"adapter_type": "monitor", "original_source": telemetry_event.source},
            )
            ...
            self.logger.debug(f"Message data: {msg.data.decode()}")
            # Send to dead letter queue
        ...
        while self.running:
            try:
                ...
                self._metrics["queue_high_water_mark"] = max(
                    self._metrics["queue_high_water_mark"], current_queue_size
                )

                # Wait for messages with adaptive timeout
                try:
                    message = await asyncio.wait_for(
                        self._message_queue.get(), timeout=self._batch_timeout
                    )
                ...
                should_process = (
                    len(batch) >= self._batch_size or time_since_last_batch >= self._batch_timeout
                )
                ...
                self.logger.error("Error in message processing loop", extra={"error": str(e)})
        ...
        for message_type, event in batch:
            try:
                ...
                self.logger.error(
                    ...
                    extra={"error": str(e), "event_id": getattr(event, "event_id", "unknown")},
                )
        ...
        self.logger.debug(
            ...
            extra={
                ...
                "throughput_msg_per_sec": (
                    round(processed / processing_time, 2) if processing_time > 0 else 0
                ),
            },
        )
    ...
    async def _start_grpc_server(self) -> None:
        ...
        try:
            # Create gRPC service
            self.grpc_service = DigitalTwinService(world_model=self.world_model, logger=self.logger)

            # Create server
            self.grpc_server = grpc.aio.server(
                futures.ThreadPoolExecutor(max_workers=self.grpc_config.get("max_workers", 10))
            )

            # Add service to server
            digital_twin_pb2_grpc.add_DigitalTwinServicer_to_server(
                self.grpc_service, self.grpc_server
            )
            ...
            self.logger.error("Failed to start gRPC server", extra={"error": str(e)})
            raise
    ...
    async def _stop_grpc_server(self) -> None:
        ...
        if self.grpc_server:
            try:
                ...
                self.logger.error("Error stopping gRPC server", extra={"error": str(e)})
            finally:
    ...
    async def get_health_status(self) -> Dict[str, Any]:
        ...
        status = {
            ...
            "world_model_initialized": (
                self.world_model.is_initialized if self.world_model else False
            ),
    ...
    async def validate_world_model(self) -> Dict[str, Any]:
        ...
        """
        if not self.world_model:
            return {"valid": False, "error": "No World Model loaded"}

        try:
            from polaris.models.world_model import WorldModelValidator

            # Run comprehensive validation
            validation_results = await WorldModelValidator.run_comprehensive_test(self.world_model)
            ...
            self.logger.error("World Model validation failed", extra={"error": str(e)})

            return error_result

    async def change_world_model_type(
        self, new_model_type: str, new_config: Optional[Dict[str, Any]] = None
    ) -> bool:
        ...
        try:
            ...
            self.logger.info(f"Changing World Model from '{old_model_type}' to '{new_model_type}'")
            ...
            if success:
                self.logger.info(f"Successfully changed World Model to '{new_model_type}'")
            else:
            ...
            self.logger.error(
                ...
                extra={"new_model_type": new_model_type, "error": str(e)},
            )
            return False
