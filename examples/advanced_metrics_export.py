...
async def simulate_framework_activity(polaris: Polaris):
    ...
    for i in range(5):
        ...
        for system_id in ["web-server", "database", "cache"]:
            polaris.metrics.increment("polaris.telemetry.collected", 
                                    tags={"system_id": system_id})
            polaris.metrics.increment("polaris.knowledge.state_stored",
                                    tags={"system_id": system_id})
    
    # Simulate strategy assessments and adaptations
    for system_id in ["web-server", "database"]:
        polaris.metrics.increment("polaris.strategy.assessments",
                                tags={"system_id": system_id})
        
        # Simulate some threshold breaches
        if system_id == "web-server":
            polaris.metrics.increment("polaris.strategy.threshold.high_threshold_exceeded",
                                    tags={"metric": "cpu_usage", "system_id": system_id})
            polaris.metrics.increment("polaris.adaptations.proposed",
                                    tags={"system_id": system_id, "action_type": "scale_up"})
            polaris.metrics.increment("polaris.adaptations.executed",
                                    tags={"system_id": system_id, "action_type": "scale_up", "status": "success"})
    
    # Simulate event bus activity
    polaris.metrics.increment("polaris.event_bus.events_published", 
                            tags={"event_type": "TelemetryEvent"}, value=15)
    polaris.metrics.increment("polaris.event_bus.events_published",
                            tags={"event_type": "AdaptationEvent"}, value=2)
...
async def main():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        total_adaptations = summary['counters'].get('polaris.adaptations.executed{action_type=scale_up,status=success,system_id=web-server}', 0)
        ...
        for hist_name, hist_data in summary['histograms'].items():
            if 'duration' in hist_name:
                print(f"  {hist_name}: avg={hist_data['avg']:.3f}s, max={hist_data['max']:.3f}s")
        
        # Show system activity
        total_telemetry = sum(v for k, v in summary['counters'].items() 
                            if k.startswith('polaris.telemetry.collected'))
