...
class PolarisInteractiveCLI(cmd.Cmd):
    ...
    Provides commands to query knowledge base, world model, and system status.
    """
...
╚══════════════════════════════════════════════════════════════╝
    ...
    def do_status(self, arg):
        ...
        # Components status
        self._print(f"Strategy: {self.polaris.strategy.__class__.__name__ if self.polaris.strategy else 'None'}")
        self._print(f"World Model: {self.polaris.world_model.__class__.__name__ if self.polaris.world_model else 'None'}")
        self._print(f"Knowledge Store: {self.polaris.knowledge_store.__class__.__name__ if self.polaris.knowledge_store else 'None'}")
        self._print(f"Meta Learner: {self.polaris.meta_learner.__class__.__name__ if self.polaris.meta_learner else 'Disabled'}")
    ...
    def do_metrics(self, arg):
        ...
        try:
            ...
            if arg:
                ...
                filtered_counters = {k: v for k, v in summary.get('counters', {}).items() 
                                   if component in k.lower()}
                filtered_gauges = {k: v for k, v in summary.get('gauges', {}).items() 
                                 if component in k.lower()}
                filtered_histograms = {k: v for k, v in summary.get('histograms', {}).items() 
                                     if component in k.lower()}
    ...
    def do_knowledge(self, arg):
        ...
        try:
            ...
            # Query states
            states = self._run_async(self.polaris.knowledge_store.query_states(
                system_id, start_time, end_time
            ))
            
            # Query actions
            actions = self._run_async(self.polaris.knowledge_store.query_actions(
                system_id, start_time, end_time
            ))
            ...
            if RICH_AVAILABLE and actions:
                ...
                for action, result in actions[-10:]:  # Show last 10
                    ...
                    exec_time = f"{result.execution_time_ms}ms" if result.execution_time_ms else "N/A"
    ...
    def do_predict(self, arg):
        ...
        args = arg.split()
        if len(args) < 2:
            self._print("Usage: predict <system_id> <action_type> [param=value ...]", style="yellow")
            return
...
def run_interactive_cli_standalone(config_path: str, cli_overrides: dict = None):
    """
...
async def run_interactive_cli(polaris):
    """
