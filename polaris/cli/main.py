...
def main() -> int:
    ...
    parser.add_argument(
        "--metrics-experiment", type=str, metavar="NAME", help="Experiment name for metrics files"
    )

    parser.add_argument(
        "--disable-metrics", action="store_true", help="Disable metrics collection entirely"
    )
    ...
    try:
        ...
        if args.dashboard:
            ...
            print("Starting Polaris with interactive dashboard")
...
async def run_framework(polaris: Polaris) -> None:
...
async def run_with_interactive_cli(polaris: Polaris) -> None:
...
async def run_with_dashboard(polaris: Polaris, clear_screen: bool = False) -> None:
