...
class ConnectorRegistry:
    ...
    async def register(self, connector: Connector) -> None:
        ...
        if self._metrics:
            self._metrics.increment("polaris.registry.connector_registered",
    ...
    def get(self, system_id: str) -> Optional[Connector]:
        ...
        if self._metrics:
            self._metrics.increment("polaris.registry.connector_accessed",
                                  tags={"system_id": system_id, 
                                        "found": str(connector is not None).lower()})
