...
Allows all components to call ``self.metrics.increment(...)`` etc.
*unconditionally* — the guards ``if self.metrics:`` are no longer needed.
...
Usage::
    ...
    metrics.increment("polaris.anything")  # silently ignored
"""
...
class NullMetricsCollector(MetricsCollector):
    ...
    Use this as a drop-in replacement for ``None`` so that components do not
    need ``if self.metrics:`` guards before every metric emission.
    """
