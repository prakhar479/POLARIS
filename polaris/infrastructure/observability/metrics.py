...
class SimpleMetricsCollector(MetricsInterface):
    ...
    def __init__(self) -> None:
        """Initialize simple in-memory metrics collector."""
    ...
    def _export_json(self, file_path: Path) -> None:
        ...
        data = {
            ...
            "last_updated": {
                k: v.isoformat() for k, v in self._last_updated.items() if isinstance(v, datetime)
            },
        }
        ...
        if rows:
            with open(file_path, "w", newline="") as f:
                writer = csv.DictWriter(
                    f, fieldnames=["metric_name", "metric_type", "value", "last_updated"]
                )
                writer.writeheader()
